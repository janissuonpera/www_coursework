...
router.post('/user/new_user', [check('username').isLength({ min: 5 }).isString().trim().escape(),
...
router.post('/user/pay', [auth.is_registered, check('card_name').isLength({min:1}).isString().trim().escape(),
                         check('card_num').isLength({min: 10}).isString().trim().escape(),
                         check('exp_date').isLength({min: 5}).isString().trim().escape(),
                         check('cvv').isLength({min:3}).isString().trim().escape(),
...
router.post('/user',[check('username').isLength({min: 5}).isString().trim().escape(),
                      check('password').isLength({min: 3}).isString().trim().escape(),
...
router.post('/admin-page/update', [auth.is_admin, check('username').isLength({min: 5}).isString().trim().escape(),
                                   check('old_username').isLength({min: 5}).isString().trim().escape(),
                                  controller.admin_update]);

//Route for when an admin wishes to remove a user's account from the database
router.post('/admin-page/delete', [auth.is_admin, check('username').isLength({min: 5}).isString().trim().escape(),
